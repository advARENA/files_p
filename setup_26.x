...
# Proxy Configuration
PROXY="http://arenaTeam:oPdxJ00ekO@94.159.114.50:81"
APT_OPTS="-o Acquire::http::Proxy=${PROXY} -o Acquire::https::Proxy=${PROXY}"
...
install_pre_reqs() {
    ...
    # Run 'apt update'
    if ! apt $APT_OPTS update -y; then
        handle_error "$?" "Failed to run 'apt update'"
    fi

    # Run 'apt install'
    if ! apt $APT_OPTS install -y apt-transport-https ca-certificates curl gnupg; then
    ...
    if ! curl -x "${PROXY}" -fsSL https://deb.nodesource.com/gpgkey/nodesource-repo.gpg.key | gpg --dearmor -o /usr/share/keyrings/nodesource.gpg; then
